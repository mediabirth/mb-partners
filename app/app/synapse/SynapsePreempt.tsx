...
export default function SynapsePreempt({ items }: { items: PreemptItem[] }) {
  ...
  return (
    <div style={{ margin: '0 20px 4px', background: '#fff', border: '1px solid var(--blue-bg)', borderRadius: 14, padding: '13px 15px' }}>
      <div style={{ fontSize: '.56rem', fontWeight: 900, letterSpacing: '.1em', color: 'var(--blue)', marginBottom: 9 }}>今日の動き</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {(ready ? shown : items.slice(0, 2)).map(i => (
          <div key={i.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            <span style={{ flexShrink: 0, fontSize: '.5rem', fontWeight: 800, color: i.badge === '先回り' ? '#fff' : 'var(--blue)', background: i.badge === '先回り' ? 'var(--blue)' : 'var(--blue-bg)', borderRadius: 5, padding: '2px 6px', marginTop: 1 }}>{i.badge}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '.66rem', color: 'var(--txt)', lineHeight: 1.6 }}>{i.text}</div>
              <div style={{ display: 'flex', gap: 13, marginTop: 5 }}>
                <Link href={i.href} prefetch={false} style={{ fontSize: '.62rem', fontWeight: 800, color: 'var(--blue)', textDecoration: 'none' }}>{i.actionLabel} →</Link>
                <button onClick={() => dismiss(i.id)} style={{ background: 'none', border: 'none', color: 'var(--muted2)', fontSize: '.62rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>後で</button>
              </div>
            </div>
          </div>
        ))}
