              ...
              <Link key={e.key} href={e.href} prefetch={e.lazy ? false : undefined} className="row-hover" style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 16px', borderTop: '1px solid var(--line)', textDecoration: 'none', color: 'var(--txt)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                    <span style={{ fontSize: '.82rem', fontWeight: 500, ...oneLine }}>{e.main}</span>
                    <span style={{ flexShrink: 0, fontSize: '.5rem', fontWeight: 500, color: dot, background: corp ? 'var(--blue-bg)' : 'var(--bg2)', borderRadius: 5, padding: '2px 6px' }}>{corp ? '法人' : '個人'}</span>
                  </div>
