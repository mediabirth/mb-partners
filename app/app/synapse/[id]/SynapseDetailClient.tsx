    ...
    <div className="page-anim" style={{ padding: '14px 0 28px' }}>
      ...
      {(() => {
        ...
        return (
          <div style={{ padding: '12px 20px 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
            ...
            <span style={{ fontSize: '.56rem', fontWeight: 500, color: 'var(--c-blue)', background: 'var(--blue-bg)', borderRadius: 4, padding: '3px 10px', flexShrink: 0, marginTop: 4 }}>{entityLabel}・見込み</span>
          </div>
        )
      })()}
      ...
      {nudge && !nudgeHidden && (
        <div style={{ margin: '12px 20px 0', display: 'flex', alignItems: 'center', gap: 9, background: 'var(--blue-bg2)', border: '1px solid var(--blue-bg)', borderRadius: 10, padding: '9px 12px' }}>
          <span style={{ flexShrink: 0, fontSize: '.5rem', fontWeight: 500, color: '#fff', background: 'var(--ink)', borderRadius: 5, padding: '2px 6px' }}>先回り</span>
              ...
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--blue-bg)' }}>
                {rescan ? (
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => scan(true)} disabled={scanBusy} className="lift" style={{ flexShrink: 0, background: 'var(--ink)', color: '#fff', border: 'none', borderRadius: 9, padding: '0 13px', cursor: scanBusy ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: '.66rem', fontWeight: 500 }}>SYNAPSE</button>
                  </div>
                ) : (
          ...
          <div style={{ textAlign: 'center', padding: '6px 0 2px' }}>
            ...
            {aiEnabled ? (
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button onClick={() => scan(false)} disabled={scanBusy} className="lift" style={{ flexShrink: 0, background: 'var(--ink)', color: '#fff', border: 'none', borderRadius: 10, padding: '0 16px', cursor: scanBusy ? 'default' : 'pointer', fontFamily: 'inherit', fontSize: '.7rem', fontWeight: 500 }}>SYNAPSE</button>
              </div>
            ) : (
      ...
      <div style={{ margin: '26px 20px 0', textAlign: 'center' }}>
        <Button variant="secondary" size="sm" href={referHref} style={{ borderRadius: 4 }}>このつながりを紹介する →</Button>
      </div>
        ...
        <div onClick={ev => { if (ev.target === ev.currentTarget) setPicked(null) }} style={{ position: 'fixed', inset: 0, background: 'rgba(14,14,20,.35)', backdropFilter: 'blur(2px)', zIndex: 125, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0 0 calc(20px + env(safe-area-inset-bottom))' }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '14px 16px', width: 'calc(100% - 40px)', maxWidth: 340, boxShadow: '0 12px 40px rgba(14,14,20,.18)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => makeIntro(picked.tag, picked.kind)} className="lift" style={{ background: 'var(--ink)', color: '#fff', border: 'none', borderRadius: 9, padding: '7px 14px', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.7rem', fontWeight: 500 }}>{picked.kind === 'service' ? 'このサービスで紹介文を作る' : 'このキーワードで紹介文を作る'}</button>
            </div>
          </div>
        </div>
      )}
