            ...
            <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', padding: '16px 20px 24px' }}>
              ...
              {navSel === 'basic' ? (
                <div>
                  ...
                  <Fld label="ブランド名"><input value={brand.name} readOnly style={{ ...inputStyle, background: 'var(--bg2)', color: 'var(--muted2)' }} /></Fld>
                  ...
                  <Fld label="説明（〜とは）（申請制）" pending={pendingOf('description')}><textarea rows={3} value={d('description:' + brand.id, brand.description ?? '')} onChange={e => setDraft(p => ({ ...p, ['description:' + brand.id]: e.target.value }))} placeholder="サービスの概要を記載" style={{ ...inputStyle, resize: 'vertical' }} /></Fld>
                  <Fld label="こんな方に（申請制）" pending={pendingOf('target_audience')}><input value={d('audience:' + brand.id, brand.target_audience ?? '')} onChange={e => setDraft(p => ({ ...p, ['audience:' + brand.id]: e.target.value }))} placeholder="例：投資用マンションを検討している人" style={inputStyle} /></Fld>
                  <Fld label="紹介対象（フック文）（申請制）" pending={pendingOf('who')}><input value={d('who:' + brand.id, brand.who ?? '')} onChange={e => setDraft(p => ({ ...p, ['who:' + brand.id]: e.target.value }))} placeholder="不動産・保険に関心のあるお客さまと接する方" style={inputStyle} /></Fld>
                  <Fld label="サービスサイトURL（申請制）" pending={pendingOf('url')}><input value={d('url:' + brand.id, brand.url ?? '')} onChange={e => setDraft(p => ({ ...p, ['url:' + brand.id]: e.target.value }))} placeholder="https://example.com" style={inputStyle} /></Fld>
                ...
                <div>
                  ...
                  <Fld label="一言説明（一覧に表示・申請制）" pending={pendingOf('menu_short_description', selMenu.id)}><input value={d('msd:' + selMenu.id, selMenu.short_description ?? '')} onChange={e => setDraft(p => ({ ...p, ['msd:' + selMenu.id]: e.target.value }))} placeholder="例：お客さまを紹介するだけ。実務は当社が対応。" style={inputStyle} /></Fld>
                  <Fld label="詳細説明（詳細シートに表示・申請制）" pending={pendingOf('menu_description', selMenu.id)}><textarea rows={3} value={d('md:' + selMenu.id, selMenu.description ?? '')} onChange={e => setDraft(p => ({ ...p, ['md:' + selMenu.id]: e.target.value }))} placeholder="例：お客さまの状況を伺い、最適なプランをご提案します" style={{ ...inputStyle, resize: 'vertical' }} /></Fld>
