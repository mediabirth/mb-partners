...
export default function SupplierSettings({ variant = 'card' }: { variant?: 'card' | 'page' } = {}) {
  ...
  return (
    <div style={{ padding: variant === 'page' ? 0 : '14px 20px 0' }}>
      <div className={variant === 'page' ? 'sup-prod-grid' : undefined} style={{ display: variant === 'page' ? 'grid' : undefined, gridTemplateColumns: '1fr', gap: 12 }}>
      {variant === 'page' && (
        <aside className="sup-prod-side">
          <div style={{ background: '#fff', border: LINE, borderRadius: 13, padding: '10px 8px', position: 'sticky', top: 66 }}>
            {data.brands.map(br => (
              <div key={br.id}>
                <a href={'#sb-' + br.id} style={{ display: 'flex', alignItems: 'center', minHeight: 38, padding: '0 10px', borderRadius: 8, fontSize: '.74rem', fontWeight: 700, color: 'var(--txt)', textDecoration: 'none' }}>{br.name}</a>
                {data.menus.filter(m => m.service_id === br.id).map(m => (
                  <a key={m.id} href={'#sm-' + m.id} style={{ display: 'flex', alignItems: 'center', minHeight: 34, padding: '0 10px 0 20px', borderRadius: 8, fontSize: '.7rem', color: 'var(--muted2)', textDecoration: 'none' }}>{m.name}</a>
                ))}
              </div>
            ))}
          </div>
      ...
      <div style={{ background: '#fff', border: LINE, borderRadius: 13, overflow: 'hidden' }}>
        {variant !== 'page' && <button onClick={() => setOpen(v => !v)} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', color: 'var(--txt)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {open && (
          <div style={{ borderTop: LINE, padding: '4px 16px 16px' }}>
            ...
            {variant === 'page' && pending.length > 0 && (
              <p style={{ fontSize: '.62rem', fontWeight: 700, color: 'var(--c-blue)', margin: '10px 0 0' }}>申請中 {pending.length} 件（MB確認待ち）</p>
            )}
            {data.brands.map(br => (
              <div key={br.id} id={'sb-' + br.id} style={{ marginTop: 14 }}>
                ...
                {data.menus.filter(m => m.service_id === br.id).map(m => (
                  <div key={m.id} id={'sm-' + m.id} style={{ borderTop: '0.5px solid var(--line)', paddingTop: 10, marginTop: 10 }}>
      ...
      <style>{`.sup-prod-side{display:none}@media (min-width:1024px){ .sup-prod-grid{grid-template-columns:220px 1fr !important} .sup-prod-side{display:block} }`}</style>
    </div>
  )
}
