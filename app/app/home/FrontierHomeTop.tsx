...
/**
 * ペルソナ・ホーム（フロンティア・非サプライヤー）: ホーム上部の「網」ヒーロー＋最優先アクション。
  ...
  return (
    <div>
      ...
      <div className="shine" style={{ margin: '18px 20px 0', background: 'linear-gradient(135deg,#5240F2 0%,#4733E6 52%,#3A28CE 100%)', color: '#fff', borderRadius: 18, padding: '20px 22px 16px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ fontSize: '.56rem', letterSpacing: '.22em', opacity: .85, textTransform: 'uppercase', marginBottom: 7 }}>あなたの網 — 今月の還元</div>
        ...
        <div style={{ display: 'flex', gap: 18, marginTop: 12, paddingTop: 11, borderTop: '1px solid rgba(255,255,255,.28)' }}>
          ...
          <a href="/app/frontier" style={{ marginLeft: 'auto', alignSelf: 'flex-end', fontSize: '.62rem', color: 'rgba(255,255,255,.9)', textDecoration: 'none' }}>網の詳細 ›</a>
        </div>
      </div>
      {/* 最優先アクション: リファラルを招待（リンク共有） */}
      <div style={{ margin: '12px 20px 0', background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: '13px 16px' }}>
        <div style={{ fontSize: '.74rem', fontWeight: 500, marginBottom: 8 }}>リファラルを招待</div>
        <FrontierInvite />
      </div>
    </div>
  )
