      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 5, background: 'var(--bg)', padding: '10px 20px 6px' }}>
        <div style={{ display: 'flex', gap: 6, background: 'var(--bg2)', borderRadius: 12, padding: 4 }}>
          {TABS.map(t => {
            const on = tab === t.k
            return (
              <button key={t.k} onClick={() => setTab(t.k)} style={{ flex: 1, padding: '8px 0', borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.74rem', fontWeight: on ? 700 : 500, background: on ? 'var(--c-blue)' : 'transparent', color: on ? '#fff' : 'var(--muted2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'background .15s' }}>
                {t.label}
                {t.badge != null && <span style={{ fontSize: '.56rem', fontWeight: 700, minWidth: 16, height: 16, padding: '0 4px', borderRadius: 999, background: on ? 'rgba(255,255,255,.25)' : 'var(--line)', color: on ? '#fff' : 'var(--muted2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{t.badge}</span>}
              </button>
            )
          })}
        </div>
...
function NextTaskCard({ task, onChange, onError }: { task: VTask; onChange: () => void; onError: (m: string) => void }) {
  ...
  return (
    <div style={{ margin: '14px 20px 0', background: 'linear-gradient(135deg,#4733E6,#3A28CE)', borderRadius: 14, padding: '14px 16px', color: '#fff', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', right: -30, top: -30, width: 110, height: 110, border: '1.5px solid rgba(255,255,255,.14)', borderRadius: '50%' }} />
      <div style={{ fontSize: '.56rem', fontWeight: 700, opacity: .85, marginBottom: 4, position: 'relative' }}>次にやること</div>
      <div style={{ fontSize: '.92rem', fontWeight: 800, letterSpacing: '-.01em', position: 'relative' }}>{task.title}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 9, position: 'relative' }}>
        {task.due_date && <span style={{ fontSize: '.6rem', opacity: .9 }}>期限 {task.due_date.slice(5).replace('-', '/')}</span>}
        {task.needs_deliverable && <span style={{ fontSize: '.52rem', fontWeight: 700, background: 'rgba(255,255,255,.2)', borderRadius: 20, padding: '2px 9px' }}>成果物が必要</span>}
        <button onClick={done} disabled={busy} style={{ marginLeft: 'auto', fontSize: '.64rem', fontWeight: 700, color: 'var(--c-blue)', background: '#fff', border: 'none', borderRadius: 8, padding: '7px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>{busy ? '…' : '完了にする'}</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '8px 20px 24px' }}>
      ...
      {doTasks.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {doTasks.map(t => {
            ...
            return (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', background: '#fff', borderRadius: 12, border: `1.5px solid ${isNext ? 'var(--c-blue)' : 'var(--line)'}`, boxShadow: isNext ? '0 2px 10px rgba(71,51,230,.08)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '.78rem', fontWeight: isNext ? 700 : 500, textDecoration: done ? 'line-through' : 'none', color: done ? 'var(--muted2)' : 'var(--txt)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.title}</div>
                  {(t.due_date || t.needs_deliverable || isNext) && (
                    <div style={{ display: 'flex', gap: 6, marginTop: 4, alignItems: 'center' }}>
                      {isNext && <span style={{ fontSize: '.5rem', fontWeight: 700, color: 'var(--c-blue)', background: 'var(--blue-bg)', borderRadius: 6, padding: '1px 6px' }}>次のタスク</span>}
                      {t.needs_deliverable && <span style={{ fontSize: '.5rem', fontWeight: 700, color: 'var(--amber)', background: 'var(--amber-bg)', borderRadius: 6, padding: '1px 6px' }}>成果物が必要</span>}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '20px 0 8px' }}>
        <h2 style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)' }}>成果物</h2>
        <button onClick={() => setShowForm(v => !v)} style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--c-blue)', background: 'var(--blue-bg)', border: 'none', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', fontFamily: 'inherit' }}>{showForm ? '閉じる' : '＋ 提出する'}</button>
      </div>
    ...
    <div style={{ padding: '12px 20px 24px' }}>
      {messages.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {groups.map(g => (
            <div key={g.date}>
              <div style={{ textAlign: 'center', margin: '2px 0 10px' }}><span style={{ fontSize: '.54rem', color: 'var(--muted2)', fontWeight: 700, background: 'var(--bg2)', borderRadius: 999, padding: '3px 12px' }}>{dayLabel(g.date)}</span></div>
...
function MoneyTab({ assignmentId, customerLabel, baseFee, delivered, expenses }: { assignmentId: string; customerLabel: string; baseFee: number; delivered: boolean; expenses: VExpense[] }) {
  return (
    <div style={{ padding: '12px 20px 0' }}>
      <h2 style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', margin: '4px 0 8px' }}>この案件の委託費</h2>
      <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: '15px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '.72rem', color: 'var(--muted2)', display: 'flex', alignItems: 'center', gap: 8 }}>
          委託費
          <span style={{ fontSize: '.5rem', fontWeight: 700, color: delivered ? 'var(--green)' : 'var(--amber)', background: delivered ? 'var(--green-bg)' : 'var(--amber-bg)', borderRadius: 20, padding: '2px 9px' }}>{delivered ? '納品済・確定' : '納品後に確定'}</span>
        </span>
        <span className="tnum" style={{ fontFamily: 'Inter', fontSize: '1.15rem', fontWeight: 800 }}>¥{baseFee.toLocaleString()}</span>
      </div>
...
function DeliverableUploader({ assignmentId, tasks, onDone, onError }: { assignmentId: string; tasks: VTask[]; onDone: () => void; onError: (m: string) => void }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 12, padding: '12px 14px' }}>
      <label style={{ display: 'block', border: `1.5px dashed ${file ? 'var(--blue)' : 'var(--line)'}`, borderRadius: 10, padding: '14px 12px', textAlign: 'center', cursor: busy ? 'not-allowed' : 'pointer', background: file ? 'var(--blue-bg2)' : 'var(--bg2)', marginBottom: 8 }}>
...
function ChatComposer({ assignmentId, onDone, onError }: { assignmentId: string; onDone: () => void; onError: (m: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: '#fff', border: '1px solid var(--line)', borderRadius: 999, padding: '5px 5px 5px 6px', boxShadow: '0 2px 10px rgba(0,0,0,.05)' }}>
