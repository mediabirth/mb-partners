  ...
  return (
    <div className="page-anim">
      ...
      <div style={{ padding: '10px 20px 6px' }}>
        <h1 style={{ fontSize: '1.06rem', fontWeight: 700, letterSpacing: '-.01em' }}>お問い合わせ</h1>
        <p style={{ fontSize: '.64rem', color: 'var(--muted2)', marginTop: 5, lineHeight: 1.6 }}>委託費・案件・アカウントなど、お困りごとを運営にお送りください。</p>
      </div>

      {sent ? (
        <div style={{ margin: '14px 20px', background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: '28px 20px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '.82rem', fontWeight: 700 }}>送信しました</div>
        ...
        <div style={{ padding: '8px 20px 28px' }}>
          <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: '16px' }}>
            <label style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', display: 'block', marginBottom: 7 }}>カテゴリ</label>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
              {CATS.map(c => {
                const on = category === c.value
                return (
                  <button key={c.value} type="button" onClick={() => setCategory(c.value)} style={{ padding: '7px 12px', borderRadius: 999, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.7rem', fontWeight: on ? 700 : 500, background: on ? 'var(--c-blue)' : 'var(--bg2)', color: on ? '#fff' : 'var(--muted2)' }}>{c.label}</button>
                )
              })}
            </div>
            <label style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', display: 'block', marginBottom: 6 }}>件名（任意）</label>
            <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="例: 委託費の振込日について" style={{ width: '100%', border: '1.5px solid var(--line)', borderRadius: 9, padding: '10px 12px', fontFamily: 'inherit', fontSize: '.78rem', marginBottom: 14, boxSizing: 'border-box' }} />
            <label style={{ fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', display: 'block', marginBottom: 6 }}>お問い合わせ内容</label>
            <textarea value={body} onChange={e => setBody(e.target.value)} rows={5} placeholder="お困りごとの内容をご記入ください。" style={{ width: '100%', border: '1.5px solid var(--line)', borderRadius: 9, padding: '11px 12px', fontFamily: 'inherit', fontSize: '.78rem', resize: 'vertical', boxSizing: 'border-box' }} />
