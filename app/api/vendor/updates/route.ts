...
export async function POST(req: NextRequest) {
  ...
  const kind = b.kind === 'flag' ? 'flag' : b.kind === 'message' ? 'message' : 'note'
  ...
  const { data, error } = await admin.from('delivery_updates').insert({
    delivery_assignment_id: assignmentId, kind, body, sender: 'vendor', status: kind === 'flag' ? 'open' : null, created_by: vendor.userId,
  }).select('id, kind, body, status, sender, created_at').single()
