...
// メッセージ司令塔 Phase3-A：テンプレート編集/削除。owner gate・service_role・隔離表のみ。
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (Array.isArray(b.attachments)) { const a = b.attachments.filter((x: { type?: string; path?: string }) => x?.type === 'image' && x?.path).slice(0, 5); patch.attachments = a.length ? a : null }
