...
export async function GET() {
  ...
  if (error || !data) {
    return NextResponse.json({ settings: MB_DEFAULTS, connected: false, ready: false })
  }
  const { oauth_tokens, ...settings } = data as Record<string, unknown>
  return NextResponse.json({ settings, connected: !!(data as any).active && !!oauth_tokens, ready: true })
}
