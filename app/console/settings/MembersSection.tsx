  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9, alignItems: 'center', marginBottom: 16 }}>
        ...
        <button onClick={invite} disabled={busy} className="ui-btn ui-btn--primary" style={{ fontSize: '.74rem', padding: '8px 16px' }}>メンバーを招待</button>
      </div>
      {loading ? <p style={{ fontSize: '.74rem', color: 'var(--muted2)' }}>読み込み中…</p> : list.length === 0 ? (
        ...
        <div style={{ border: '0.5px solid var(--line)', borderRadius: 10, overflow: 'hidden' }}>
          {list.map((m, i) => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 14px', borderBottom: i < list.length - 1 ? '0.5px solid var(--line)' : 'none' }}>
