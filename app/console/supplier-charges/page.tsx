'use client'
/**
 * サプライヤー請求（P0-a・仕様正典: docs/design/lineage-rate-design.md v2）。
 * MBが「請求する」側の独立ビュー＝支払管理（MBが払う）とは厳密分離。
 * 月次クローズ（金額の凍結・第2段）＋状態管理（unbilled→invoiced→settled）＋CSV。Phase 0=手動請求。
 */
...
import { GUIDE_SUPPLIER_CHARGES } from '@/lib/console-guides'
...
const KIND_JP: Record<string, string> = { half_commission: '折半手数料（粗利50%）', payment_fee_5: '決済手数料（5%）', omnis_monthly: '月額（ファウンディング）' }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg2)' }}>
      ...
      <ConsoleMain>
        <div className="console-topbar" style={{ background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(10px)', borderBottom: '0.5px solid var(--line)', padding: '13px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 30 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}><h1 style={{ fontSize: '1rem', fontWeight: 500 }}>サプライヤー請求</h1><PageGuide data={GUIDE_SUPPLIER_CHARGES} /></span>
          <Link href="/console/payouts" className="ui-btn ui-btn--secondary" style={{ fontSize: '.7rem', padding: '7px 14px', textDecoration: 'none' }}>支払管理（MBが払う側）→</Link>
        </div>

        <div style={{ padding: '24px 28px 44px', maxWidth: 1040 }}>
          {loading ? <div className="ui-skeleton" style={{ height: 120, borderRadius: 14 }} /> : suppliers.length === 0 ? (
            <div style={{ background: '#fff', border: '0.5px solid var(--line)', borderRadius: 14, padding: '48px 24px', textAlign: 'center', color: 'var(--muted2)', fontSize: '.78rem' }}>
              サプライヤーが未登録です。サービスマスタでブランドに supplier（会社フロンティア）を紐づけると、ここに請求が集計されます。
            </div>
          ) : (
            <>
              {/* 月次クローズ */}
                ...
                return (
                  <div key={key} style={{ background: '#fff', border: '0.5px solid var(--line)', borderRadius: 14, overflow: 'hidden', marginBottom: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '11px 14px', background: 'var(--bg2)', fontSize: '.72rem', fontWeight: 700 }}>
                      <span>{p} ・ {nameOf(sid)}</span><span style={{ fontFamily: 'Inter' }}>計 {yen(total)}（税抜）</span>
                    </div>
