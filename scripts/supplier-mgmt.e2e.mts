...
console.log('[1.5] ペルソナ: 昇格前=フロンティアのホーム（網ヒーロー・商品なし）')
{
  ...
  ok(t15.includes('あなたのチーム — 今月の還元')&&t15.includes('仲間を招待'),'昇格前: チームヒーロー＋招待アクション（v3語彙）')
...
ok(((await pg.evaluate(`document.body.innerText`)) as string).includes('ファウンディング'),'一覧にfoundingで出現')
{
  ...
  ok(t25.includes('今月の売上（成約受注額）')&&t25.includes('今月のお支払い見込み'),'昇格直後: サプライヤー・コンソールへ切替（ゼロ状態）')
...
console.log('[5] UI: フロンティア紐づけ→系統内案件(決済5%)→成約')
...
ok(d2?.fee_snapshot?.self_service===true&&d2?.fee_snapshot?.rate_kind==='payment_fee_5'&&Number(d2?.fee_snapshot?.rate)===0.05,'fee_snapshot=系統内→決済5%',JSON.stringify(d2?.fee_snapshot))
console.log('RESULT-A: '+pass+'/'+(pass+fail)+' errors='+JSON.stringify(errs.slice(0,3)))
if(fail>0)process.exit(1)

console.log('[6] UI: 受注額入力(ドロワー)→月次クローズ→請求出現（決済5%＋折半）')
...
const half=chg?.find((x:any)=>x.kind==='half_commission'); const pay5=chg?.find((x:any)=>x.kind==='payment_fee_5')
ok(Number(half?.amount)===100000,'凍結: 折半100,000',JSON.stringify(chg))
ok(Number(pay5?.amount)===500,'凍結: 決済手数料500')
const frozenHash=JSON.stringify((chg??[]).map((x:any)=>[x.kind,x.amount]).sort())
const frozenBaseHash=JSON.stringify((chg??[]).filter((x:any)=>x.kind!=='payment_fee_5'&&x.kind!=='passthrough_revenue_fee').map((x:any)=>[x.kind,x.amount]).sort())
...
ok(d2b?.fee_snapshot?.rate_kind==='payment_fee_5'&&d2b?.fee_snapshot?.rate_card_version==='omnis-founding-v1','deal2の凍結条件は不変(foundingカードの決済5%)')
...
for(const path of ['/app','/app/refer','/app/rewards']){await lin.goto(BASE+path,{waitUntil:'domcontentloaded'});await lin.waitForTimeout(2200)
  ...
  for(const w of ['サプライヤー','レートカード','折半','override','決済手数料'])if(t.includes(w))leak+=path+':'+w+' '}
...
ok(JSON.stringify((chg3??[]).filter((x:any)=>!newKinds.has(x.kind)).map((x:any)=>[x.kind,x.amount]).sort())===frozenBaseHash,'既凍結行（折半）は不変')
...
for(const vp of [{width:375,height:667},{width:1024,height:768}]){
  ...
  if(vp.width===375){
    ...
    ok(t.includes('ダッシュボード')&&!t.includes('商品'),'[375] v9: ダッシュボード改名＋商品語ゼロ')
    // ドロワーでナビ全項目
    ...
    ok(dt.includes('ダッシュボード')&&dt.includes('パートナー')&&dt.includes('サービスマスタ')&&dt.includes('案件')&&dt.includes('お金')&&dt.includes('設定')&&dt.includes('供給 検証'),'[375] ドロワー: ナビ全項目（ダッシュボード改名）＋チップ')
    ...
    ok(tm.includes('① MB Partnersへのお支払い')&&tm.includes('② パートナーへの報酬')&&tm.includes('③ 委託先への委託費')&&tm.includes('お受け取り'),'[375] お金v8: 3区分＋お受け取り')
    ok(tm.includes('販売手数料')&&!tm.includes('折半')&&!tm.includes('5%）')&&tm.includes('15,000'),'[375] お金v5: 平易ラベル（条件語なし）＋実数15,000')
    ok(!tm.includes('月額')&&tm.includes('決済手数料'),'[375] お金: 月額表記なし＋決済手数料')
    ok(tm.includes('締め済み・請求書待ち'),'[375] お金: 対外語彙')
    ok(tm.includes('③ 委託先への委託費'),'[375] お金: 委託区分')
    ...
    ok(tn.includes('パートナーを招待')&&tn.includes('委託先を招待')&&tn.includes('今月生んだ売上'),'[375] パートナーv9: topbar招待2種＋KPI')
    ok(tn.includes('委託先（実務を担う方）'),'[375] v9: パートナー＋委託先を1画面統合（タブなし）')
...
ok(pb.includes('あなたのチーム — 今月の還元'),'パターンb: ホーム上部がチームヒーロー')
ok(!pb.includes('今月の売上（成約受注額）')&&!pb.includes('お支払い見込み'),'パターンb: サプライヤー・コンソール要素なし')
...
console.log('[19] craft検査（v2是正の実測）')
{
  const pcx=await b.newContext({viewport:{width:1280,height:900}})
  const pp=await pcx.newPage()
  await pp.goto(BASE+'/app/login',{waitUntil:'domcontentloaded'});await pp.waitForTimeout(1200)
  await pp.locator('input[type="email"]').fill(SUPMAIL);await pp.locator('input[type="password"]').fill(PW)
  await pp.locator('button[type="submit"]').first().click();await pp.waitForTimeout(3000)
  await pp.goto(BASE+'/app',{waitUntil:'domcontentloaded'});await pp.waitForTimeout(2500)
  ...
  for(const p of ['/app','/app/s/network','/app/s/products','/app/s/deals','/app/s/money','/app/s/settings']){
    await pp.goto(BASE+p,{waitUntil:'domcontentloaded'});await pp.waitForTimeout(2000)
    const t=await pp.evaluate(`document.body.innerText`) as string
    if(t.includes('網')||t.includes('リファラル'))vocabBad+=p+' '
    if(t.includes('リンクを共有するだけ')||t.includes('お客さまのページに表示されます（')||t.includes('請求書に記載の口座へお願いします'))vocabBad+=p+':常設文 '
  }
  ok(vocabBad==='','品質ゲート①②: 網/リファラル・常設説明文=rendered全ページゼロ',vocabBad)
  // 分掌: 設定=会社（ログイン情報カード不在・マイページ相互リンク）／マイページ=個人（口座は設定へ）
  await pp.goto(BASE+'/app/s/settings',{waitUntil:'domcontentloaded'});await pp.waitForTimeout(2500)
  const ts9=await pp.evaluate(`document.body.innerText`) as string
  ok(ts9.includes('マイページ →')&&!ts9.includes('ログインメール'),'分掌: 設定=会社のこと＋マイページ相互リンク')
  await pp.goto(BASE+'/app/mypage',{waitUntil:'domcontentloaded'});await pp.waitForTimeout(2500)
  const tm9=await pp.evaluate(`document.body.innerText`) as string
  ok(tm9.includes('設定で管理しています')&&tm9.includes('設定 →'),'分掌: マイページ=個人＋口座は設定へ相互リンク')
  // 招待フォーム=網ページの1箇所のみ（rendered）
  let formPages=0
  for(const p of ['/app','/app/s/network','/app/s/products','/app/s/deals','/app/s/money','/app/s/settings']){
    await pp.goto(BASE+p,{waitUntil:'domcontentloaded'});await pp.waitForTimeout(2000)
    const t=await pp.evaluate(`document.body.innerText`) as string
...
console.log('[20] v6: 商品ドロワー可視・案件ボード・委託先（招待→アサイン→整合）')
{
  ...
  const drawerTxt=await p6.evaluate(`document.body.innerText`) as string
  ok(drawerTxt.includes('基本情報')&&drawerTxt.includes('サブタイトル')&&drawerTxt.includes('変更を申請'),'サービスマスタv8: 全フィールド＋二層動詞')
  ...
  // 委託先: 招待（リンクのみ・実メール0）
  await p6.goto(BASE+'/app/s/partners',{waitUntil:'domcontentloaded'});await p6.waitForTimeout(2500)
  await p6.locator('button',{hasText:'委託先を招待'}).click();await p6.waitForTimeout(800)
  await p6.locator('input[placeholder="例：山田保険事務所"]').fill('CC設備サービス')
  await p6.locator('input[placeholder="例：保険の実務"]').fill('保険の実務')
  ...
  await p6.locator('button',{hasText:'閉じる'}).click().catch(()=>{});await p6.waitForTimeout(600)
  await p6.goto(BASE+'/app/s/partners',{waitUntil:'domcontentloaded'});await p6.waitForTimeout(2600)
  const dvListTxt=await p6.evaluate(`document.body.innerText`) as string
  ok(dvListTxt.includes('CC設備サービス')&&dvListTxt.includes('招待済み・未登録'),'委託先: 一覧に状態表示',dvListTxt.match(/委託先[\s\S]{0,120}/)?.[0].replace(/\n+/g,'/'))
  // アサイン: 案件ドロワーから提示
  await p6.goto(BASE+'/app/s/deals',{waitUntil:'domcontentloaded'});await p6.waitForTimeout(2800)
