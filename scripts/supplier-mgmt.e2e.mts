...
for(const vp of [{width:375,height:667},{width:1024,height:768}]){
  ...
  if(vp.width===375)ok(sp.url().includes('/app/dashboard'),'[375] 旧URL /app/supplier→統合ダッシュボードへリダイレクト',sp.url())
  const t=await sp.evaluate(`document.body.innerText`) as string
  if(vp.width===375){
    ok(t.includes('今月のハイライト')&&t.includes('会社の成約'),'[375] 統合ヒーロー（役割横断）')
    ok(t.includes('あなたの会社')&&t.includes('あなたの網')&&t.includes('あなたの紹介'),'[375] セクション①②③④（supplier=全部・パターンc/d）')
  ...
  if(vp.width===375){
    ...
    ok(mt.includes('ダッシュボード')&&!mt.includes('サプライヤー ポータル')&&!mt.includes('フロンティア ダッシュボード'),'[375] mypage: 導線1本化（ダッシュボード）')
  }
  await sctx.close()
}
...
ok(lin.url().includes('/app/dashboard'),'パターンb: フロンティアのみ→到達')
ok(pb.includes('あなたの網')&&pb.includes('あなたの紹介')&&!pb.includes('あなたの会社'),'パターンb: ③④のみ（②なし）')
await pg.goto(BASE+'/console/partners/'+linId,{waitUntil:'domcontentloaded'});await pg.waitForTimeout(2500)
