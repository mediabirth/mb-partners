...
ok(((await pg.evaluate(`document.body.innerText`)) as string).includes('ファウンディング'),'一覧にfoundingで出現')
{
  ...
  ok(t25.includes('今月の全体像')&&t25.includes('成約受注額'),'昇格直後: ホームがミニコンソールへ切替（ゼロ状態）')
  ok(t25.includes('会社'),'昇格直後: ナビに会社タブ')
...
for(const vp of [{width:375,height:667},{width:1024,height:768}]){
  ...
  if(vp.width===375){
    ok(t.includes('今月の全体像')&&t.includes('成約受注額')&&t.includes('お支払い見込み'),'[375] ミニコンソールヒーロー（全体像1列）')
    ok(t.includes('商品')&&t.includes('案件とお金')&&t.includes('網の動き')&&t.includes('サービス設定'),'[375] セクション（商品/案件とお金/網の動き・設定入口）パターンc/d')
    ok(t.includes('リファラルを招待')&&t.includes('紹介する'),'[375] 最優先アクション帯')
...
ok(!pb.includes('今月の全体像')&&!pb.includes('サービス設定'),'パターンb: ミニコンソール要素なし')
