...
console.log('[7] UI: サプライヤー詳細=請求/系統/ブランド表示→カード付け替え(std-v1)')
...
const cardSet=await pg.evaluate(`(()=>{const sels=[...document.querySelectorAll('select')];for(const s of sels){const o=[...s.options].find(o=>o.value==='std-v1');if(o){const set=Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,'value').set;set.call(s,'std-v1');s.dispatchEvent(new Event('change',{bubbles:true}));return true}}return false})()`)
...
ok(supRow?.supplier_rate_card==='std-v1','カード付け替え=UI操作でstd-v1へ',JSON.stringify(supRow))
...
ok((ev??[]).some((x:any)=>x.event==='card_changed'&&x.from_card==='omnis-founding-v1'&&x.to_card==='std-v1'),'履歴: card_changed記録',JSON.stringify(ev))
...
ok(d3?.fee_snapshot?.rate_kind==='payment_fee_5'&&Number(d3?.fee_snapshot?.rate)===0.05&&d3?.fee_snapshot?.rate_card_version==='std-v1','deal3=新カード(決済5%/std-v1)',JSON.stringify(d3?.fee_snapshot))
