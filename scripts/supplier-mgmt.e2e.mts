...
for(const vp of [{width:375,height:667},{width:1024,height:768}]){
  ...
  if(vp.width===375){
    ...
    ok(tn.includes('＋ パートナーを招待')&&tn.includes('今月生んだ売上'),'[375] パートナーv8: topbar招待＋KPI')
...
console.log('[20] v6: 商品ドロワー可視・案件ボード・委託先（招待→アサイン→整合）')
{
  ...
  await p6.goto(BASE+'/app/s/partners?tab=delivery',{waitUntil:'domcontentloaded'});await p6.waitForTimeout(2600)
