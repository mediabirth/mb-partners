...
for(const vp of [{width:375,height:667},{width:1024,height:768}]){
  ...
  if(vp.width===375){
    ok(t.includes('今月の売上（成約受注額）')&&t.includes('進行中の案件')&&t.includes('網のメンバー')&&t.includes('今月のお支払い見込み'),'[375] ホームv2: 数字4枚')
    ok(t.includes('次の一手')&&t.includes('今月の流れ'),'[375] 次の一手＋今月の流れ')
    ok(t.includes('網を広げる')&&!t.includes('招待リンクを作成'),'[375] 網カード=ボタンのみ（フォーム不在・§0b）')
    // ドロワーでナビ全項目
    ...
    ok(td.includes('CCE2E-A')&&td.includes('あなたの網')&&td.includes('MB Partnersの紹介'),'[375] 案件: テーブル＋紹介系統表示（v2語彙）')
    ...
    ok(tn.includes('招待リンクを作成してコピー')&&tn.includes('あなたの網が今月生んだ売上'),'[375] 網v2: リンク主体招待＋真実の売上ヒーロー')
    ok(!tn.includes('12ヶ月')&&!tn.includes('貢献額')&&!/月 ¥[\d,]+ の(オーバーライド)?見込み/.test(tn),'[375] 網v2: 12ヶ月/貢献額/試算の不在')
...
console.log('[19] craft検査（v2是正の実測）')
{
  ...
  ok(side?.hasMypage===false,'サイドバー: マイページ撤去（§0f・設定へ統合）')
  ...
  ok(ts.includes('振込先口座')&&ts.includes('ログイン情報'),'設定v2: 口座＋ログイン情報の統合')
  await pcx.close()
}
