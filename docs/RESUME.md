...
### 2026-08-18 戦略裁定「紹介が生まれない問題」→ GEN-5「信用」設計確定

- 勝彦の根本疑問「MBを知らない人が紹介料目当てで紹介するか」→リード調査回答=**構造的にNO**（紹介=評判の貸し出し・金は評判リスクを補償できない）。国内外事例の教訓: 動く紹介は①隣接業型(freee/HubSpot/Shopify=本業利益)②信用裏書き型(地銀マッチング)③相互扶助型(BNI=金銭なし35万人)。Saleshub/side bizz型（不特定多数の紹介副業）は歩留まり問題ごと輸入になるため**採用しない**。
- リード提言を勝彦採択: **主エンジン=隣接業パートナー型（MB版=共同受注ネットワーク・高さんモデルの複製）＋弾=ブランド別無料診断オファー＋潤滑油=相互扶助（当面プロダクト化しない）**。「100人招待」は「隣接業10人」へ再定義。
- GEN-5設計書 docs/design/gen5-trust.md v1 確定: ①entry_offers（診断3種・/r/入口・共有シート診断カード・OG拡張）②case_studies（**同意フラグ必須をDB check制約で強制**・0件は節不在）③紹介者保護の両面明文化＋**/r/フッターに紹介料開示1行**（ステマ規制対応=「機構を透けさせない」原則の部分改定・勝彦へ明示報告）④共同受注率をNorth Star隣に追加。
- 勝彦の手番（プロダクト外）: 初期実績3-5件+掲載同意・隣接業10人リスト。
...
- GEN-4設計書 docs/design/gen4-northstar.md v1 確定。核心=①新ページなし=/console/growth冒頭にNorth Star節（2回目紹介率=2回以上÷1回以上・n=併記・分母0は「—」・1→2回目中央値日数）②**起票時src捕捉で帰属ループ完成**（deals.src additive・/api/referralへ透過のみ・reward_snapshot/帰属/凍結は1行も不変・報酬ハッシュ定義外）③GENループ週次表8週（digest送信/閲覧/card閲覧/共有/起票/src別起票/成約=WON定義はsocial-proofとre-export共有で二重定義禁止）④パートナー3ステージ（owner面につき実名可・0件の人も見せる）。
- 検証の要点: reward_snapshot全ビット同形式の機械比較・deals hashのE2E中変動→復帰証明・ペルソナの真実=psql全桁突合。

### 2026-08-18 GEN-3 検収合格＋リード引き取りデプロイ（bc38b38）＝「ネットワークの鼓動」点灯

- GEN-2と同じ引き取り型: Codexは実装+単体+build green・環境制約でE2E/デプロイ保留→リードがclone取り込み（FF・bc38b38）から本番まで完遂。レポートは新形式（1コードブロック）で受領=コピー運用成立。
- diff精査: 集計はid/状態/時刻/除外識別子のみselect（money列不接触）・文面生成の入力を3件数+期間に型固定・全0はSSRでDOM不在（スケルトンなし=CLSなし）・digest行は同一libで二重化なし・障害時は0表示でなく省略の安全側縮退。wins判定文字列は現行writer（console deals STATUS_LABEL.confirmed=成約確定）と一致確認済み。
- **副産物の既存ズレ発見**: lib/public-timeline.ts 許可表に旧語彙「ステータスを「成約」に変更しました」が残存（現writerは「成約確定」）→公開タイムラインが成約遷移を拾えない可能性。GEN-3非接触のため別タスク化（spawn済み・要修正）。
- 検証: 単体12/12・test:gen3実走（30日/7日窓の差分・独立直接集計と全桁一致・原状復帰・残置0）・test:verify**初回フルGREEN**。
- デプロイ dpl_6QkBRgtpXQjbtUy3RRYNSeEumPFn・stamp bc38b38=HEAD実測。本番実ブラウザ375px: カード出現「新しい仲間が 3人 加わりました」・**ペルソナの真実=psql直接集計3と全桁一致**・溢れ0・pageerror 0（証跡=docs/reports/gen3_screens/gen3-home-375-prod.png）。7日窓は全0=digest証明行は正しく省略される状態。
- money 4ハッシュ+seed 38/340,320 前後完全一致・CCGEN3残置0（psql実測）。
- **新教訓（メモリ保存済み）**: service-role clientでverifyOtpを呼ぶと以後RLS化する（本番突合の誤検知で発見・render-refer.mjsも同構造）→セッション鋳造は別anonクライアント規律に。
- **GEN-1〜3が全て本番に揃った**。残るはGEN-4「計測」=2回目紹介率ダッシュボード。

### 2026-08-18 GEN-3「火」設計確定（勝彦GO・完全自走）

- GEN-3設計書 docs/design/gen3-socialproof.md v1 確定。核心=①ネットワーク集計のみ（紹介/成約/新メンバー×件数×期間・個人特定粒度と金額は構造遮断）②抑制ルール=0行は出さない・全0は面ごと消す（コールドスタートで空虚を見せない・盛らない）③表示先はAPPホーム「ネットワークの鼓動」カード+週次ダイジェスト1行の2箇所のみ（顧客面には出さない）④決定的生成・ペルソナの真実=SQL全桁突合を合格条件に。
- 勝彦指示の追加恒久化: Codexレポートは**1コードブロックのプレーンテキスト**（コピーでそのまま渡せる形・表組み禁止）→AGENTS.md「統合レポートの型」に追記。
...
- 22メニューの決定的ローテーション、新着優先、同一人物の連週重複防止を純粋関数+canonへ配線。停止リンクとAPP/supplierのメール通知は同じ`member_notification_prefs.email_enabled`を使用。
- console配信面に既定OFFトグル、次回対象/3分類、3文面、履歴を追加。375px検証で同面だけヘッダ用classが欠けていた既存干渉を検出し最小修理（横溢れ0・操作子44px）。
- 専用E2E: 3セグメント、禁止語0、同週2回→1回、停止後除外、OFF完全無音、無署名401、auth含む残置0。`pnpm test:verify`は全green。実配信トグルはOFFを維持。

## （旧）セッション引き継ぎ（2026-08-06）
...
### 2026-08-06 invite-polish（リード直接実装・5e5dd7e）＋🎌開業儀式成立の突合

- **勝彦指摘3点の是正（リード実装）**: ①「サプライヤー（会社）」→「サプライヤー」②作成ボタンをui-btn--mdで統一 ③登録ウィザードのサプライヤー分岐=会社名欄（招待社名プリフィル）+ご担当者姓名の分離+区分は法人固定+確認画面に会社名行。
- **実バグ根治**: サプライヤー登録で partners.company_name が未保存だった（デモはseed由来）→accept APIに保存+corporateサーバ側強制（§7-2）。API直叩き実測=会社名保存/法人強制/担当者名分離 全✓。
- 検証: ウィザード実走（供給者UI全✓）・test:verify=フル3回中1・2回目が console復帰1件red（idle5=1,874ms）→単独再走21/21×2→**3回目フルGREEN**で確定。session 32/32×3（第5条件）。本番stamp 5e5dd7e=HEAD・タグpush・自動デプロイ不発。
- **正直な開示（リードの手戻り3件）**: ①旧検証サーバのポート4599居座り（pkillがnext-serverに不効）で編集前ビルドを3プローブ誤検証→ポート基準killへ手順改定 ②cwd誤りでフル走行2回が空振り ③プローブ残置1件（supplier_card_eventsのFK連鎖でdeleteUser不発）→連鎖ごと完全撤去・残置0確認。
- **バックログ追加**: resume-perfのフル走行時マージン問題（console復帰がスイート後半負荷で単発赤・単独green）→計測隔離（resume-perf先行実行 or 専用サーバ）の検証基盤バッチ。
- **🎌開業儀式の突合（実データ）**: ZZ4172=高明さん（サプライヤー・omnis-founding-v1・法人・8/5 23:53）／ZZ8240=若菜優哉さん（リファラル・上位=高さん系統・8/6 02:06）＝**千秋楽成立・系統リンク正常**。card_events 8/5 11:16=ZZ4820（勝彦の自己テスト昇格）。高さんの正式社名=**株式会社オムニスリガーレ**（勝彦回答 2026-08-06）を backfill 済み（UPDATE 1行・console供給元表示で実測確認）。デモの「株式会社オムニス」とは別名＝推測せず確認した判断が正解だった記録。
