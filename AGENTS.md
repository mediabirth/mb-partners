...
- **勝彦** = 最終決定者・実機審判。**バッチ実行中に勝彦へ質問・承認待ちをしない（完全自走・2026-07-24 勝彦指示／2026-08-18 再強調: 途中の確認・承認要求は一切禁止。迷ったら安全側に倒して続行し、判断内容をレポートに記載）**。
- **リード**（Claude Code セッション）= 設計・裁定・バッチ発注・検収。判断に迷う点は勝彦でなく**リードへの報告事項として統合レポートに記す**（実装は安全側に倒して続行）。
- **Codex（あなた）** = 実装者。**実行モデルは GPT-5.6 Sol を使用すること（2026-07-24 勝彦指定・レポートの実行モデル欄に明記）**。1バッチ=1ミッション。完了時に**統合レポート全文をチャットへ必ず貼付**（レポート未達のままデプロイした前科=perf-red-fix を繰り返さない）。

## 絶対規律（違反はバッチ不合格）

1. **money 4ハッシュ**: バッチ開始時に menu_rewards／deals reward／fee／override の4ハッシュ（定義は CLAUDE.md 検証標準5）＋MB seed補助（**38行/sum=340,320**・2026-08-05改定）をスナップショットし、終了時・デプロイ後に一致を証明。**deals は 2026-08-04 以降「生き値」**＝実運用の正当な操作で変わるため、変化した場合は「誰の操作か」（created_by・partner・時刻）を必ず突合して報告する。CC/Codex 起源の throwaway は復元証明（before/after 全桁一致）必須。
...
**形式（2026-08-18 勝彦指示）**: レポート全文を**1個のコードブロック（プレーンテキスト）**として出力すること＝勝彦がそのままコピーしてリードに渡せる形。表組みは崩れるため使わず、`項目: 値` の行形式・箇条書きで書く。ブロック外に本文を書かない。
