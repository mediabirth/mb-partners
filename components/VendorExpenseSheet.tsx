...
export default function VendorExpenseSheet({ open, onClose, presetAssignmentId, presetLabel }: {
  ...
  if (!open) return null
  const inp: React.CSSProperties = { width: '100%', border: '1.5px solid var(--line)', borderRadius: 10, padding: '11px 13px', fontFamily: 'inherit', fontSize: '.86rem', background: '#fff' }
  const lbl: React.CSSProperties = { display: 'block', fontSize: '.64rem', fontWeight: 700, color: 'var(--muted2)', margin: '0 0 6px' }

  return (
    <>
      <style>{`
        .ves-scrim{position:fixed;inset:0;background:rgba(14,14,20,.4);backdrop-filter:blur(2px);z-index:90;animation:vesf .2s ease}
        .ves-sheet{position:fixed;left:0;right:0;bottom:0;z-index:95;background:#fff;border-radius:20px 20px 0 0;padding:14px 20px calc(22px + env(safe-area-inset-bottom));box-shadow:0 -16px 48px rgba(14,14,20,.22);animation:vesu .26s cubic-bezier(.2,.8,.2,1);max-height:92dvh;overflow-y:auto}
        ...
        @media (min-width:520px){.ves-sheet{left:50%;transform:translate(-50%,-50%);top:50%;bottom:auto;width:440px;border-radius:18px;max-height:88dvh;animation:vesf .2s ease}}
      `}</style>
      ...
      <div className="ves-sheet" role="dialog" aria-label="経費を申請">
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          {KINDS.map(k => (
            <button key={k} onClick={() => setKind(k)} disabled={busy} style={{ flex: 1, padding: '9px 0', borderRadius: 10, fontFamily: 'inherit', fontSize: '.78rem', fontWeight: 700, cursor: 'pointer', border: `1.5px solid ${kind === k ? 'var(--blue)' : 'var(--line)'}`, background: kind === k ? 'var(--blue-bg)' : '#fff', color: kind === k ? 'var(--blue)' : 'var(--txt)' }}>{k}</button>
          ))}
        </div>
        ...
        {preview ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, border: '1.5px solid var(--line)', borderRadius: 12, padding: 10, marginBottom: 16 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={preview} alt="プレビュー" style={{ width: 54, height: 54, objectFit: 'cover', borderRadius: 8 }} />
            <span style={{ flex: 1, fontSize: '.72rem', fontWeight: 600, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{file?.name}</span>
            <button onClick={() => pick(null)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '.9rem' }}>✕</button>
          </div>
        ) : file ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, border: '1.5px solid var(--line)', borderRadius: 12, padding: '12px 12px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <button onClick={() => camRef.current?.click()} disabled={busy} className="btn btn-g" style={{ flex: 1, justifyContent: 'center', gap: 7 }}>
            ...
            <button onClick={() => fileRef.current?.click()} disabled={busy} className="btn btn-g" style={{ flex: 1, justifyContent: 'center', gap: 7 }}>
        ...
        <button onClick={submit} disabled={busy || !assignmentId} className="btn btn-p" style={{ width: '100%', justifyContent: 'center', padding: '13px', fontSize: '.86rem' }}>{busy ? '送信中…' : '申請する'}</button>
      </div>
      {toast && <div style={{ position: 'fixed', bottom: 'calc(20px + env(safe-area-inset-bottom))', left: '50%', transform: 'translateX(-50%)', background: 'var(--txt)', color: '#fff', padding: '12px 22px', borderRadius: 10, fontSize: '.74rem', fontWeight: 600, zIndex: 130 }}>{toast}</div>}
    </>
  )
}
