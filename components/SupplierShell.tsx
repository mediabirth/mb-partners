 ...
 * ナビ: ホーム／網（リファラル）／商品／案件／お金／設定（マイページはサイドバーから撤去＝設定へ統合・§7）。
 */
...
function BrandHead({ companyName }: { companyName: string }) {
  return (
    <div style={{ padding: '16px 16px 12px', borderBottom: '0.5px solid var(--line)' }}>
      <div style={{ fontSize: '.82rem', fontWeight: 700, letterSpacing: '.1em', color: 'var(--c-blue)' }}>MB Partners</div>
      <div style={{ fontSize: '.66rem', color: 'var(--muted2)', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{companyName}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg2)' }}>
      ...
      <header className="sup-mobilehead" style={{ position: 'sticky', top: 0, zIndex: 40, background: 'rgba(255,255,255,.94)', backdropFilter: 'blur(10px)', borderBottom: '0.5px solid var(--line)', display: 'flex', alignItems: 'center', gap: 8, padding: '0 14px', height: 52 }}>
        ...
        <span style={{ fontSize: '.78rem', fontWeight: 700, letterSpacing: '.1em', color: 'var(--c-blue)' }}>MB Partners</span>
      ...
      <div style={{ display: 'flex' }}>
        ...
        <aside className="sup-side" style={{ width: 230, flexShrink: 0, borderRight: '0.5px solid var(--line)', background: '#fff', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
          <BrandHead companyName={companyName} />
          <NavList />
        </aside>
        ...
        <div className="sup-drawer-wrap">
          ...
          <div className="drawer-in" style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 264, maxWidth: '82vw', background: '#fff', zIndex: 95, boxShadow: '8px 0 30px rgba(14,14,20,.18)', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ flex: 1 }}><BrandHead companyName={companyName} /></div>
