...
export default function InviteFellowCard({ partnerId }: { partnerId: string }) {
  ...
  return (
    <div style={{ margin: '0 20px', background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: '16px 16px' }}>
      ...
      {/* M-3：コピーを主アクション（上の控えめなCOPY）。メール/LINE はセカンダリ＝テキストリンク寄りで控えめに。onClick/共有先は不変。 */}
      <div style={{ display: 'flex', gap: 14, alignItems: 'center', justifyContent: 'center', marginTop: 4 }}>
        <button onClick={shareMail} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', padding: '4px 4px' }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="M3 7l9 6 9-6" /></svg>
          メールで送る
        </button>
        <span aria-hidden style={{ width: 1, height: 12, background: 'var(--line)' }} />
        <button onClick={shareLine} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '.66rem', fontWeight: 700, color: 'var(--muted2)', padding: '4px 4px' }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3C6.5 3 2 6.6 2 11c0 3.9 3.5 7.2 8.3 7.9.3.07.7.2.8.5.07.27.05.7.02.97l-.13.8c-.04.24-.2.94.82.51 1.02-.43 5.5-3.24 7.5-5.55C20.6 14.9 22 13.1 22 11c0-4.4-4.5-8-10-8z" /></svg>
          LINEで送る
        </button>
      </div>
    </div>
