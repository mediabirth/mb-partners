...
export type SettingsNoti = { title: string; desc: string; state?: 'on' | 'soon'; control?: React.ReactNode }
...
export default function SettingsScreen({ links, notifications, logout, extra }: {
  ...
  return (
    <div className="page-anim">
      ...
      <div style={{ margin: '6px 20px 14px', background: '#fff', border: '0.5px solid var(--line)', borderRadius: 13, overflow: 'hidden' }}>
        {links.map((l, i) => <SettingsRow key={l.href} href={l.href} last={i === links.length - 1}>{l.label}</SettingsRow>)}
      </div>

      {/* アプリ（PWA）— app/vendor 共通 */}
      <div style={{ padding: '2px 24px 8px', fontSize: '.68rem', color: 'var(--muted)', fontWeight: 500 }}>アプリ</div>
      ...
      <div style={{ padding: '2px 24px 8px', fontSize: '.68rem', color: 'var(--muted)', fontWeight: 500 }}>通知</div>
      <div style={{ margin: '0 20px 14px', background: '#fff', border: '0.5px solid var(--line)', borderRadius: 13, overflow: 'hidden' }}>
        {notifications.map((n, i) => <NotiRow key={n.title} title={n.title} desc={n.desc} state={n.state} control={n.control} last={i === notifications.length - 1} />)}
      </div>
