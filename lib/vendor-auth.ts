...
import { createServiceRoleClient, getCachedUser, getCachedProfile } from '@/lib/supabase/server'
...
export const resolveVendor = cache(async (): Promise<{ userId: string; deliveryId: string; deliveryName: string } | null> => {
  ...
  // profiles(role) と deliveries(auth_user_id) は user.id のみ依存・順序非依存 → 並列取得。
  const [profile, { data: delivery }] = await Promise.all([
    getCachedProfile(),
    admin.from('deliveries').select('id, name').eq('auth_user_id', user.id).maybeSingle(),
  ])
